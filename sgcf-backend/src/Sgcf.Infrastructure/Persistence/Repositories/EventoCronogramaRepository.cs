...
internal sealed class EventoCronogramaRepository(SgcfDbContext context) : IEventoCronogramaRepository
{
    ...
    public async Task<IReadOnlyList<EventoCronograma>> ListAtrasadosAntesDeAsync(
        LocalDate hoje,
        CancellationToken cancellationToken)
    {
        List<EventoCronograma> list = await context.EventosCronograma
            .Where(e => e.Status == StatusEventoCronograma.Atrasado
                     && e.DataPrevista < hoje)
            .ToListAsync(cancellationToken);
        return list.AsReadOnly();
    }

    public async Task<IReadOnlyList<EventoCronograma>> ListPrevistosNoPeriodoAsync(
        LocalDate dataInicio,
        LocalDate dataFim,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<IReadOnlyList<EventoCronograma>> ListPorTiposAsync(
        IReadOnlyCollection<TipoEventoCronograma> tipos,
        CancellationToken cancellationToken = default)
    {
        List<EventoCronograma> list = await context.EventosCronograma
            .Where(e => tipos.Contains(e.Tipo)
                     && e.Status != StatusEventoCronograma.Cancelado
                     && e.Status != StatusEventoCronograma.Refinanciado)
            .ToListAsync(cancellationToken);
        return list.AsReadOnly();
