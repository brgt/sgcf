...
| AD-3 | **Cronograma da simulação on-the-fly** — não persistido. Calculado a cada consulta via `CronogramaStrategyFactory` existente. | Simulação muda muito durante refinamento. Persistir gera drift e reconciliação. Custo: O(N) eventos por simulação por consulta — aceitável para 12 meses. |
