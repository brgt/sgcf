...
**Stack:** **.NET 11** + ASP.NET Core + EF Core 11 + NodaTime + PostgreSQL (Cloud SQL) + Cloud Run + GCP `southamerica-east1`
...
**Cronograma absoluto (v1.5):** **11/maio/2026 → 20/novembro/2026** (28 semanas calendário descontando feriados)
**Plano tático de execução:** `tasks/execucao_tatica.md` v1.0 — Fase B + Sprint 0 + Sprint 1 dia a dia
...
| Item                     | Decisão                                                                   | Origem         |
| ------------------------ | ------------------------------------------------------------------------- | -------------- |
| Stack                    | **.NET 11** + EF Core 11 + NodaTime + PostgreSQL 16                       | ADR-003 (v1.2) |
| Hospedagem               | GCP, `us-central1`, Cloud Run                                             | ADR-005        |
...
#### 0.3 Solution .NET 11 + estrutura Clean Architecture

**Descrição:** criar repositório git, solution `.sln` com 7 projetos (Domain / Application / Infrastructure / Api / Mcp / A2a / Jobs) + tests. Configurar EF Core 11 + NodaTime + migrations Code First.
...
**Descrição:** tabela conforme Anexo B seção 6.3, com 12 tipos de evento (PRINCIPAL, JUROS, IRRF*RETIDO, IOF_CAMBIO, COMISSAO*_, TARIFA\__, BREAK_FUNDING_FEE, MULTA_MORATORIA). Campo `numero_evento` agrupa parcelas da mesma data.
...
| --- | ----------------------------------------------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **1 dev como SPOF em sistema financeiro complexo**                            | Alta × Crítico  | Documentação contínua, pair-programming com sponsor em decisões-chave, reforço pontual orçado, frameworks opinionated (.NET 11 + libs maduras) — ADR-002, ADR-007 |
...
| 11  | ~~**.NET 11 ainda em preview no início da Sprint 0**~~ **RISCO ENCERRADO**    | —               | Decisão confirmada pelo sponsor em 10/maio/2026: usar .NET 11. Código iniciado em net11.0. |
...
| 11  | ~~**Versão definitiva do .NET**~~ — **DECIDIDO: .NET 11** (10/mai/2026, sponsor)                  | ~~Sponsor + dev~~       | ~~Final Fase B~~ ✅      |
...
- **v1.5 — 10/maio/2026**: inclusão do **plano tático de execução** (`tasks/execucao_tatica.md`):
  - Calendário absoluto fixado: kickoff em **11/maio/2026**, go-live em **20/novembro/2026**
  - 8 milestones com datas (M0-M8) consolidados
  - Plano dia a dia da Fase B (4 semanas), Sprint 0 (10 dias úteis) e Sprint 1 (10 dias úteis)
  - Cadência de cerimônias e métricas de acompanhamento definidas
  - 10 riscos das primeiras 8 semanas com owner e mitigação
  - 8 decisões críticas (D1-D8) com defaults se não decididas a tempo
  - Onboarding do dev (~10h estruturadas)
  - Premissas confirmadas pelo sponsor: início imediato, .NET 11 condicional, backend puro 3 primeiros meses
